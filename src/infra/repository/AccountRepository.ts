...
export class AccountRepositoryMemory implements AccountRepository {
    ...
    public async updateActive(id: string, active: boolean): Promise<void> {
        const index = this.accounts.findIndex(item => item.accountId === id);
        if (index !== -1) this.accounts[index].setActive(active);
    }
